...
module.exports = {
  ...
  getAllProductPaginate: async ({ activePage, limit }) => {
    try {
      ...
      const listProduct = await _Product.find().sort({ createdAt: -1 }).skip(skip).limit(limit);
      return {
        listProduct,
        totalPage,
        skip
  ...
  getProductPaginate: async ({ textSearch, activePage, limit }) => {
    try {
      ...
      const totalPage = Math.ceil(totalRecord / limit);
      const skip = (activePage - 1) * limit;
      const listProduct = await _Product.find({ name: { $regex: textSearch, $options: 'i' } }).sort({ createdAt: -1 }).skip(skip).limit(limit);
      return {
        listProduct,
        totalPage,
        skip
